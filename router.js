const databaseURL = `https://myblog-77e69-default-rtdb.firebaseio.com/`;
...
const router = async (fullPath) => {
    ...
    if (Boolean(localStorage.getItem('auth'))) {
        ...
        try {
            ...
            Object.keys(items).forEach(key => {
                if (items[key].creatorId == user.localId) {
    ...
    //root
    let root = document.getElementById('root-main');            // here we handle the root element of the HTML
